      ...
      <div style={{
        ...
        {total > 0 && (
          <div style={{ height: 4, background: 'var(--border)' }}>
            <div style={{ height: '100%', width: `${(completedCount / total) * 100}%`, background: 'var(--green)', transition: 'width 0.3s' }} />
          </div>
        )}
              ...
              {tasks.length === 0 && (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <div style={{ color: 'var(--muted)', fontSize: '0.85rem', marginBottom: 20 }}>
                    {hasDefaults ? `Load default tasks for "${operation.vessel_status}" or add your own.` : 'Add tasks below.'}
                  </div>
                  {hasDefaults && (
                    <button onClick={() => initTasks(operation.vessel_status)} className="btn-primary btn-sm">
                      Load default tasks for {operation.vessel_status}
                    </button>
                  )}
                </div>
              )}
...
function KanbanCard({ op, onEdit, onTasks, onTrack, color }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 10, padding: '14px 16px', borderTop: `3px solid ${isOverdue ? 'var(--red)' : color}` }}>
      {/* Overdue banner */}
      {isOverdue && (
        <div style={{ background: 'var(--red-bg)', color: 'var(--red)', fontSize: '0.75rem', fontWeight: 700, padding: '5px 10px', borderRadius: 6, marginBottom: 10 }}>
          ⚠️ ETD overdue by {Math.abs(etdDays)} day{Math.abs(etdDays) !== 1 ? 's' : ''}
        </div>
      )}
      ...
      {/* Port */}
      {op.port && (
        <div style={{ fontSize: '0.82rem', color: 'var(--muted)', marginBottom: 8 }}>📍 {op.port}</div>
      )}

      {/* ETA / ETD with countdown */}
      ...
      {/* Client */}
      {op.client_name && (
        <div style={{ fontSize: '0.8rem', color: 'var(--muted)', marginBottom: 6 }}>👤 {op.client_name}</div>
      )}

      {/* Sub-agent */}
      {op.sub_agent && (
        <div style={{ fontSize: '0.78rem', color: 'var(--muted)', marginBottom: 10 }}>🤝 {op.sub_agent}</div>
      )}

      {/* Buttons */}
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={() => onTrack(op)} className="btn-secondary btn-sm" style={{ flex: 1, fontSize: '0.75rem' }}>🚢</button>
        <button onClick={() => onTasks(op)} className="btn-secondary btn-sm" style={{ flex: 1, fontSize: '0.75rem' }}>📋 Tasks</button>
        <button onClick={() => onEdit(op)} className="btn-secondary btn-sm" style={{ flex: 1, fontSize: '0.75rem' }}>✏️ Edit</button>
      </div>
    </div>
  )
}
...
export default function Operational() {
  ...
  const { data: allOps, loading: loadingAll, refetch: refetchAll } = useOperations({
    ...filters,
    ...(search ? { search } : {}),
  })
  ...
  // Group active vessels by status, filtered by operator
  ...
  const grouped = KANBAN_COLUMNS.reduce((acc, col) => {
    acc[col.status] = filteredActive.filter(v => v.vessel_status === col.status)
    return acc
  }, {})
      ...
      <div style={{ marginBottom: 24 }}>
        <div className="page-header" style={{ marginBottom: 16 }}>
          ...
          <span style={{ color: 'var(--muted)', fontSize: '0.82rem' }}>
            {loadingActive ? '…' : `${filteredActive.length} active`}
          </span>
        </div>
        ...
        {loadingActive ? <div className="spinner" /> : totalActive === 0 ? (
          <div className="card empty">
            <div className="empty-icon">⚓</div>
            <p>No active vessels right now.</p>
          </div>
        ) : (
            ...
            {KANBAN_COLUMNS.map(col => {
              ...
              return (
                <div key={col.status}>
                  ...
                  {/* Cards */}
                  {vessels.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--muted)', fontSize: '0.82rem', border: '2px dashed var(--border)', borderRadius: 8 }}>
                      No vessels
                    </div>
                  ) : (
                    vessels.map(op => (
                      <KanbanCard
                        key={op.id} op={op} color={col.color}
                        onEdit={setEditOp} onTasks={setTaskOp} onTrack={setTrackOp}
                      />
                    ))
                  )}
                </div>
        ...
        {loadingAll ? <div className="spinner" /> : allOps.length === 0 ? (
          ...
          <>
            <div style={{ fontSize: '0.78rem', color: 'var(--muted)', marginBottom: 8 }}>
              {allOps.length} result{allOps.length !== 1 ? 's' : ''}
            </div>
            <div className="card">
                  ...
                  <tbody>
                    {allOps.map(op => (
                      <tr key={op.id} style={{ cursor: 'pointer' }} onClick={() => setEditOp(op)}>
                        ...
                        <td style={{ whiteSpace: 'nowrap', fontWeight: 600, color: op.net >= 0 ? 'var(--green)' : 'var(--red)' }}>
                          {op.net != null ? formatMoney(op.net, op.inv_currency) : '—'}
                        </td>
                        <td onClick={e => { e.stopPropagation(); setTrackOp(op) }}>
