...
export default function Vessels() {
  ...
  const typeColor = (type) => {
    if (!type) return { bg: 'var(--gray-bg)', color: 'var(--muted)' }
    ...
    return { bg: 'var(--gray-bg)', color: 'var(--muted)' }
  }
                ...
                {data.map(v => {
                  ...
                  return (
                    <tr key={v.id} style={{ cursor: 'pointer' }} onClick={() => openEdit(v)}>
                      ...
                      <td>
                        {v.vessel_type ? (
                          <span style={{ background: tc.bg, color: tc.color, fontSize: '0.72rem',
                      ...
                      <td onClick={e => { e.stopPropagation(); del(v.id) }}
                        style={{ color: 'var(--muted)', cursor: 'pointer', fontSize: '0.85rem' }}>🗑</td>
                    </tr>
                  )
                })}
              </tbody>
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--navy)', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: '#fff', fontWeight: 600, fontSize: '0.95rem', marginTop: 2 }}>
                  {selected ? selected.name : 'New vessel'}
                </div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
              ...
              <div className="form-grid" style={{ marginBottom: 16 }}>
                ...
                <div className="form-group">
                  ...
                  <input value={form.imo || ''} onChange={e => set('imo', e.target.value)} style={{ fontFamily: 'var(--mono)' }} placeholder="9307580" />
                </div>
                <div className="form-group">
                  <label className="form-label">MMSI</label>
                  <input value={form.mmsi || ''} onChange={e => set('mmsi', e.target.value)} style={{ fontFamily: 'var(--mono)' }} placeholder="257801000" />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input value={form.flag || ''} onChange={e => set('flag', e.target.value)} placeholder="Norway" />
                </div>
                <div className="form-group form-full">
