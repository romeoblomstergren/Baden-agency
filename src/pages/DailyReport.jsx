...
function VesselSection({ op }) {
  const text = useVesselReport(op)
  return <>{text}</>
}
...
export default function DailyReport() {
  ...
  // Build manual report text from active ops (without hooks per op — just basic info)
  const manualReport = header + '\n' +
    active.map(op => {
      const etLine = [
        op.eta && 'ETA: ' + formatDate(op.eta) + (op.eta_ampm ? ' ' + op.eta_ampm : ''),
        op.etb && 'ETB: ' + formatDate(op.etb),
        op.etd && 'ETD: ' + formatDate(op.etd) + (op.etd_ampm ? ' ' + op.etd_ampm : ''),
      ].filter(Boolean).join(' \u00b7 ')
      return [
        '\u2500'.repeat(60),
        ...
        'Port      : ' + (op.port || '\u2014'),
        'Principal : ' + (op.client_name || '\u2014'),
        op.commodity ? 'Cargo     : ' + op.commodity + (op.quantity ? ' \u2014 ' + op.quantity : '') : '',
        etLine ? 'Timeline  : ' + etLine : '',
        op.notes ? '\nNotes: ' + op.notes : '',
      ].filter(Boolean).join('\n')
    }).join('\n\n') + '\n' + footer
  ...
  const openMail = () => {
    ...
    window.location.href = 'mailto:?subject=' + encodeURIComponent(subject) + '&body=' + encodeURIComponent(displayReport)
  }

  const generateAI = async () => {
    const result = await ask(
      'Write a professional daily operations report for Baden Agency for ' + dayName + ', ' + today + '.\nRaw data:\n\n' + manualReport + '\n\nRewrite as a polished professional shipping agency daily report. Keep all facts accurate.' + (reportNotes ? '\nAdditional notes: ' + reportNotes : ''),
      { system: 'You are a senior shipping agent at Baden Agency. Write professionally, concisely, in standard maritime report format.', max_tokens: 2000 }
    )
    ...
    <div className="page" style={{ maxWidth: 900 }}>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={generateAI} disabled={aiLoading || active.length === 0} className="btn-primary btn-sm">
            {aiLoading ? '\u25cc Generating\u2026' : '\u2746 AI Polish'}
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{aiMode ? 'Editable' : 'Edit notes above to customize'}</span>
            </div>
            <pre style={{ padding: 20, fontFamily: 'var(--mono)', fontSize: '0.78rem', lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: 'var(--text)', margin: 0, maxHeight: 600, overflowY: 'auto' }}>
              {displayReport}
            </pre>
          </div>
        </>
