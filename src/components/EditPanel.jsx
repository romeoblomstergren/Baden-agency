...
Use correct maritime terminology. Be specific with vessel details provided.`
  ...
  const PROMPTS = {
    ...
    opa: `Write an OPA (Owner's Protecting Agent) appointment letter for:\nVessel: ${operation.vessel_name}\nPort: ${operation.port}\nPrincipal: ${operation.client_name}\nRef: ${operation.ref}`,
    nor: `Write a Notice of Readiness (NOR) tender email for:\nVessel: ${operation.vessel_name}\nPort: ${operation.port}\nCommodity: ${operation.commodity || 'cargo'}\nETB: ${operation.etb ? formatDate(operation.etb) : 'TBC'}\nRef: ${operation.ref}`,
    proforma: `Write a proforma disbursement account (DA) covering email for:\nVessel: ${operation.vessel_name}\nPort: ${operation.port}\nPrincipal: ${operation.client_name}\nRef: ${operation.ref}`,
    etd: `Write a departure/ETD notification email for:\nVessel: ${operation.vessel_name}\nPort: ${operation.port}\nPrincipal: ${operation.client_name}\nETD: ${operation.etd ? formatDate(operation.etd) : 'TBC'}\nRef: ${operation.ref}`,
  }
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: '5%', left: '50%', transform: 'translateX(-50%)', zIndex: 301, width: '90%', maxWidth: 580, background: 'var(--surface)', borderRadius: 16, boxShadow: '0 8px 40px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', maxHeight: '90vh', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 20, flex: 1, overflowY: 'auto' }}>
          ...
          <button onClick={generate} disabled={loading} className="btn-primary btn-sm" style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{position:'fixed',top:0,right:0,bottom:0,width:'100%',maxWidth:520,background:'var(--surface)',zIndex:201,display:'flex',flexDirection:'column',boxShadow:'-4px 0 24px rgba(0,0,0,0.15)',animation:'slideIn 0.22s ease'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:20}}>
          ...
          {activeTab === 0 ? (<>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:20}}>
              <div className="form-group"><label className="form-label">IMO</label><input value={form.imo||''} onChange={e=>set('imo',e.target.value)} style={{fontFamily:'var(--mono)'}} placeholder="e.g. 9876543"/></div>
              <div className="form-group"><label className="form-label">MMSI</label><input value={form.mmsi||''} onChange={e=>set('mmsi',e.target.value)} style={{fontFamily:'var(--mono)'}} placeholder="e.g. 123456789"/></div>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:20}}>
              ...
              <div className="form-group"><label className="form-label">Add. services</label>
